...
# FIXME он используется только в get_queryset методах в нескольких местах
#  скорее всего, это можно было бы засунуть в миксину какую-то, чтобы он отдельно не болтался
#  - у каждой вьюхи есть свой класс формы и свои параметры
#  - на каждый запрос ты берешь форму класса
#  - инитишь её из запроса
#  - передаёшь ещё и параметры в autofilter
#  почитай про миксины, позволит избавится от дублирвоание логики и будет выглядеть поприятнее
...
# FIXME не стоит смешивать функциональные и class-based вьюхи
#  если ты решил писать на функциях, то пиши все.
#  но лучше на классах, конечно
#  т.к. если появится потребность повесить какие-то метрики (например),
#  то тебе придется функциональные и class-based вьюхи по разному обрабатывать
#  (для функций нужны будут декораторы, а для CBV - миксины)
#  в этом нет особо большой проблемы, но лучше все писать единообразно изначально
